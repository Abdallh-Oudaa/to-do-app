...
class TaskDao {
  ...
  static Future<List<Task>> getTasksFromFireStore(
      String uid) async {
  ...
  static Future<void> editIsDone(String uid,Task task)async{
    return getTasksCollection(uid).doc(task.id).update({"isDone":task.isDone});
  }
  static Future<void> editTask(String uid,Task task)async{
